-- C-05 : rendre les suppressions reellement reversibles.
--
-- Chaine du defaut :
--   1. supprimer une page appelle register_note_deletions, qui renseigne archived_at
--      et conserve la ligne pour permettre un retour en arriere ;
--   2. get_app_payload filtre "where archived_at is null", donc la page archivee est
--      absente du payload serveur, donc absente de la fusion faite par sync_app_payload ;
--   3. sync_app_payload_legacy_v59 terminait par un balayage
--      "delete from public.notes where slug not in (temp_notes)", qui detruisait
--      definitivement la ligne archivee des la synchronisation suivante ;
--   4. restore_deleted_notes ne trouvait alors plus aucune ligne a restaurer.
--
-- Cette migration redefinit la fonction a l'identique, avec la seule condition
-- "and archived_at is null" ajoutee au balayage final.
